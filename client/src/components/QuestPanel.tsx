...
export default function QuestPanel({ quests, gameState }: QuestPanelProps) {
  return (
    <div className="w-full lg:w-72 p-4 space-y-4">
      <h3 className="text-xl font-display font-bold">Your Quests</h3>
      <div className="space-y-3">
        {quests.map((quest) => {
          ...
          return (
            <Card
              key={quest.id}
              className={`p-4 ${isComplete ? 'bg-chart-3/10 border-chart-3' : ''} ${isITGuru ? 'bg-gradient-to-br from-chart-3/20 to-chart-4/20' : ''}`}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {isITGuru && <Badge variant="default" className="bg-chart-3">Legendary</Badge>}
                  </div>
